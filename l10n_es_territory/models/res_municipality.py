...
class ResMunicipality(models.Model):
    ...
    def _compute_cadastral_code(self):
        for record in self:
            ...
            if (
                record.province_id
                and record.province_id.cadastral_code
                and record.municipality_number
            ):
                cadastral_code = str(record.province_id.cadastral_code).zfill(
        ...
        for record in self:
            if record.cadastral_code:
                ...
                if other_municipality:
                    raise exceptions.ValidationError(_("Repeated municipality code."))
