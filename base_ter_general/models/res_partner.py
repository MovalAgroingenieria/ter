...
class ResPartner(models.Model):
    ...
    def _check_base_connection_host(self):
        ...
        if self.base_connection_host and not (
            ...
            raise exceptions.ValidationError(
                _("The host must begin with 'http://' or 'https://' [%s]")
                % self.base_connection_database
            )
    ...
    def _check_base_connection_port(self):
        if self.base_connection_port and (
            self.base_connection_port < 1 or self.base_connection_port > 65535
        ):
            raise exceptions.ValidationError(
                _("The port must be between 0 (no port) and 65535 [%s]")
                % self.base_connection_database
            )
    ...
    def _check_connection(self):
        ...
        host, port, company_id, database, username, user_password = (
        ...
        try:
            ...
            raise exceptions.UserError(
                _("The database %s does not exist [%s]") % (database, self.name.strip())
            )
        if not uid:
            raise exceptions.UserError(
                _("Authentication into database %s failed.") % database
            )
    ...
    def map_base_entity(self, cronjob=False):
        ...
        for record in self:
            if not record.mapped_to_base:
                continue
            models, uid, user_password, database, company_id = (
        ...
        for record in self:
            ...
            if not partner_code:
                raise exceptions.UserError(
                    _("Partner code is not set. Can not be mapped to base entity.")
                )

            # Get Jinja2 template
            config = self.env["ir.config_parameter"].sudo()
            jinja2_template = config.get_param(
                "base_ter_general.jinja2_template", False
            )
            if not jinja2_template:
                raise exceptions.UserError(
                    _("Jinja2 template not configured in Territory parameters.")
                )
            ...
            _logger.info(
                "Mapping: Base entity %s [%s]", record.name.strip(), partner_code
            )
            ...
            for parcel in parcels_info:
                ...
                existing_parcel = TerParcel.search(
                    [("name", "=", parcel["parcel_code"])], limit=1
                )
            ...
            if success_mapped_property_mismatch > 0:
                _logger.info(
                    "Mapping: Parcels mapped property mismatch: %d",
            ...
            if base_mapping_results:
                success_mapped_in_base_entity, failed_mapped_in_base_entity = (
            ...
            if success_mapped_property_mismatch > 0:
                message_log += (
                    _("<br/>· Parcels mapped property mismatch: %d")
                    % success_mapped_property_mismatch
                )
            if parcel_not_found > 0:
                message_log += (
            ...
            if success_mapped_in_base_entity > 0:
                message_log += (
                    _("<br/>· Parcels successfully mapped in base entity: %d")
                    % success_mapped_in_base_entity
                )
            if failed_mapped_in_base_entity > 0:
                message_log += (
    ...
    def map_base_entity_cron(self):
        primary_partners = self.env["res.partner"].search(
            [("mapped_to_base", "=", True)]
        )
