...
def pre_init_hook(cr):
    ...
    env.cr.execute(
        "ALTER DATABASE " + env.cr.dbname + " SET search_path = public, postgis"
    )
...
def post_init_hook(cr, registry):
    ...
    env["ir.config_parameter"].set_param("base_ter.area_unit_is_ha", True)
    env["ir.config_parameter"].set_param("base_ter.area_unit_name", "ha")
    env["ir.config_parameter"].set_param("base_ter.area_unit_value_in_ha", 1)
    env["ir.config_parameter"].set_param("base_ter.warning_diff_areas", 10)
    env["ir.config_parameter"].set_param(
        "base_ter.same_parcelmanager_propertyowner", False
    )
    env["ir.config_parameter"].set_param(
        "base_ter.aerial_image_wmsbase_url", "https://www.ign.es/wms-inspire/pnoa-ma"
    )
    env["ir.config_parameter"].set_param(
        "base_ter.aerial_image_wmsbase_layers", "OI.OrthoimageCoverage"
    )
    env["ir.config_parameter"].set_param(
        "base_ter.aerial_image_wmsvec_url", "https://gis.moval.es/wms/my_client"
    )
    env["ir.config_parameter"].set_param(
    ...
    env["ir.config_parameter"].set_param("base_ter.aerial_image_height", 512)
    env["ir.config_parameter"].set_param("base_ter.aerial_image_zoom", 1.2)
    env["ir.config_parameter"].set_param(
        "base_ter.gis_viewer_url", "https://gis.moval.es/my_client/visor"
    )
    env["ir.config_parameter"].set_param("base_ter.gis_viewer_username", "tecnico")
    env["ir.config_parameter"].set_param("base_ter.gis_viewer_password", "my_password")
    env["ir.config_parameter"].set_param("base_ter.gis_viewer_epsg", 25830)
    env["ir.config_parameter"].set_param(
...
def uninstall_hook(cr, registry):
    ...
    try:
        env.cr.savepoint()
        env.cr.execute("DELETE FROM ir_config_parameter " "WHERE key LIKE 'base_ter.%'")
        env.cr.commit()
    except Exception:
        env.cr.rollback()
