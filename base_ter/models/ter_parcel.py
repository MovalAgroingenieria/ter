...
class TerParcel(models.Model):
    ...
    def _compute_diff_areas_threshold_exceeded(self):
        config = self.env["ir.config_parameter"].sudo()
        warning_diff_areas = int(config.get_param("base_ter.warning_diff_areas", 0))
        for record in self:
    ...
    def _compute_address_data(self):
        for record in self:
            ...
            if (
                record.place_id
                and record.place_id.name.lower() != record.municipality_id.name.lower()
            ):
        ...
        if same_parcelmanager_propertyowner:
            for record in self:
                if (
                    ...
                    raise exceptions.ValidationError(
                        _(
                            "The parcel manager and the property manager must "
                            "be the same person."
                        )
                    )
                ...
                for partnerlink in record.partnerlink_ids:
                    if partnerlink.is_main:
                        if partnerlink.partner_id != record.partner_id:
                            raise exceptions.ValidationError(
                                _(
                                    "The parcel manager and the main "
                ...
                if profile_ids:
                    ...
                    for profile in profile_ids:
                        if profile.requires_total:
                            partnerlinks_of_profile = self.env[
                                "ter.parcel.partnerlink"
                            ].search(
                                [
                                    ("parcel_id", "=", record.id),
                            ...
                            if partnerlinks_of_profile:
                                total_percentage = sum(
                                    partnerlink_of_profile.percentage
                                    for partnerlink_of_profile in partnerlinks_of_profile
                                )
                                if total_percentage != 100:
                                    raise exceptions.ValidationError(
                                        _(
                                            "Review the profile percentages: "
...
class TerParcelPartnerlink(models.Model):
    ...
    @api.depends(
        "parcel_id", "parcel_id.alphanum_code", "partner_id", "partner_id.partner_code"
    )
    def _compute_name(self):
        for record in self:
            ...
            if record.parcel_id:
                ...
                name = record.parcel_id.alphanum_code + "-" + partner_code_asstr
            record.name = name
    ...
    def _check_partner_id(self):
        for record in self:
            if record.parcel_id and record.partner_id:
                partners_mapped_to_partnerlink = self.env[
                    "ter.parcel.partnerlink"
                ].search(
                    [
                ...
                if (
                    ...
                    and len(partners_mapped_to_partnerlink)
                ) > 1:
    ...
    def write(self, vals):
        if len(self) == 1:
            ...
            if "profile_id" in vals or "percentage" in vals:
                profile = None
                if "profile_id" in vals:
                    profile = self.env["ter.profile"].browse(vals["profile_id"])
                else:
