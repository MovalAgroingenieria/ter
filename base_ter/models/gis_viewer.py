...
class GisViewer(models.AbstractModel):
    ...
    def action_gis_viewer(self):
        ...
        gis_viewer_url = config.get_param("base_ter.gis_viewer_url", False)
    ...
    def action_gis_viewer_global(self):
        ...
        if not gis_viewer_url:
            gis_viewer_url = self.DEFAULT_GIS_VIEWER
        gis_viewer_url = gis_viewer_url + "?arg=" + self._get_encrypted_credentials()
        return {
            "type": "ir.actions.act_url",
            "url": gis_viewer_url,
            "target": "new",
    ...
    def _get_encrypted_credentials(self):
        config = self.env["ir.config_parameter"].sudo()
        gis_viewer_username = config.get_param("base_ter.gis_viewer_username", False)
        gis_viewer_password = config.get_param("base_ter.gis_viewer_password", False)
        resp = ""
    ...
    def _get_gis_link(self, public=True, minimal=False):
        ...
        if self.mapped_to_polygon:
            config = self.env["ir.config_parameter"].sudo()
            gis_link = config.get_param("base_ter.gis_viewer_url", False)
            ...
            gis_link = (
                gis_link
                + "?"
                + self.__class__._param_gis_selection
                + "="
                + self.gis_code
            )
            xmin, ymin, xmax, ymax = self._get_bounding_box()
