            ...
            data.update(
                {
                    "parcel_id": parcel.id,
                    "munici": (
                        parcel.municipality_id.display_name
                        if parcel.municipality_id
                        else ""
                    ),
                    "area": (
                        parcel.area_official_m2 if parcel.area_official_m2 else 0.0
                    ),
                    "area_gis": (parcel.area_gis if parcel.area_gis else 0.0),
                    "area_official": parcel.area_official,
                    "area_unit": parcel.area_unit_name,
                    "place_name": (
                        parcel.place_id.display_name if parcel.place_id else ""
                    ),
                    "partnerlinks": partnerlinks,
                    "off_code": (parcel.official_code if parcel.official_code else ""),
                }
            )
        if source == "gis":
            data.update(
